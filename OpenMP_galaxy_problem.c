...
//  and so on until     89.75 <=  angle  <= 90.0
    ...
    void calcangles(float rasc_1[], float dec_1[], float rasc_2[], float dec_2[], long int histogram[360]) {
        int i, j, tid, bin;
        float innerexpr, result;
        #pragma omp parallel shared(histogram) private(i, j, tid, bin, innerexpr, result)
        {
            /* Get thread number */
            ...
            if (tid == 0) {
                ...
                printf("Executing parallel section, number of threads = %d\n", nthreads);
            }
            ...
            for (i = 0; i < 100000; i++) {
                for (j = 0; j < 100000; j++) {
                    ...
                    if (innerexpr > 1.0f) {innerexpr = 1.0f;}
    ...
    calcangles(real_rasc, real_decl, real_rasc, real_decl, histogram_DD);
    calcangles(real_rasc, real_decl, rand_rasc, rand_decl, histogram_DR);
    calcangles(rand_rasc, rand_decl, rand_rasc, rand_decl, histogram_RR);
